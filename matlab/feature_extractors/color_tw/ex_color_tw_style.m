...
addr_feature = '..\..\..\data\features\color_tw\';
...
for i = 1:length(paintings)
    if(exist([addr_feature, 'features_style\', paintings{i} , '_color_tw.mat'],'file'))
        load([addr_feature, 'features_style\', paintings{i} , '_color_tw.mat']);
        if(sum(isnan(gbcolor)) == 0 && sum(isnan(lccolor)) == 0)
            disp(i);
            continue;
        end
    end
    ...
    [gbcolor, lccolor] = color_tw(img);

    % Save color temperature and weight as files
    save([addr_feature, 'features_style\', paintings{i} , '_color_tw.mat'], 'gbcolor', 'lccolor');
    disp(i);
end
