...
% the histograms of all paintings in global variable 'paintings_by_genre' 
...
for i = 1:length(paintings)
    if(exist([addr_feature, 'features_genre\', paintings{i} , '_tamura_hd.mat'],'file'))
        disp(i);
        continue;
    end
%     paintings{i} = paintings{i}(1:strfind(paintings{i},'.jpg')-1);
    % Load image
    load([addr_feature, 'perpixel_features_genre\',paintings{i}, '_tamura_perpixel.mat']);
    ...
    save([addr_feature, 'features_genre\', paintings{i} , '_tamura_hd.mat'], 'hd_crs', 'hd_con', 'hd_dir');
    disp(i);
end
