% DESCRIPTION:The extracted RGB histogram features of all paintings in 
...
%   ..\..\data\features\rgb_hist\pca_by_style\*_pca.mat
...
addr_pca = '..\..\data\features\rgb_hist\pca_by_style\';
...
for i = 1:length(styles)
    for j = 1:length(styles)
        if(i < j)
            ...
            rgbhd_pca = load([addr_pca,s1,'_',s2,'_pca.mat']);
            grp = rgbhd_pca.grp;
            rgbhd_pca = rgbhd_pca.fs_obs;
            
%             % Naive Bayes
%             f1s(i,j) = cl_nb(rgbhd_pca,grp,s1,s2);

            % K-Nearest Neighbor
            f1s(i,j) = cl_knn(rgbhd_pca,grp,10,s1,s2);
            
%             % Support Vector Machine
%             f1s(i,j) = cl_svm(rgbhd_pca,grp,'rbf',s1,s2);

%             % Classification Tree
%             f1s(i,j) = cl_tree(rgbhd_pca,grp,s1,s2);
        end
    end
end
save([addr_rst,'style\rgbhd_f1_measure.mat'],'f1s');
